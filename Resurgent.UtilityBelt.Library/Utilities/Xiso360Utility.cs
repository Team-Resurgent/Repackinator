...
using Resurgent.UtilityBelt.Library.Utilities.XgdModels;
using System;
...
{
    public static class Xiso360Utility
    {
        ...
        private static XgdHeader? GetXgdHeaer(byte[] sector)
        {
            ...
            var header = StructUtility.ByteToType<XgdHeader>(sectorReader);
            return header;
        }
        ...
        public static XgdInfo? GetXgdInfo(IImageInput input)
        {
            var found = false;
            var maxSize = input.TotalSectors * XGD_SECTOR_SIZE;
            var baseSector = 0U;
            ...
            {
                var sector = input.ReadSectors(XGD_MAGIC_SECTOR_XDKI, 1);
            ...
            {
                var sector = input.ReadSectors(XGD_MAGIC_SECTOR_XGD3, 1);
                header = GetXgdHeaer(sector);
                if (header != null && header.MagicString.Equals(XGD_IMAGE_MAGIC) && header.MagicTailString.Equals(XGD_IMAGE_MAGIC))
                {
            ...
            {
                var sector = input.ReadSectors(XGD_MAGIC_SECTOR_XGD2, 1);
                header = GetXgdHeaer(sector);
                if (header != null && header.MagicString.Equals(XGD_IMAGE_MAGIC) && header.MagicTailString.Equals(XGD_IMAGE_MAGIC))
                {
        ...
        public static bool GetFileInfoFromXiso(IImageInput input, Action<FileInfo> info, Action<float>? progress, CancellationToken cancellationToken)
        {
            if (progress != null)
            {
                progress(0);
            ...
            var xgdInfo = GetXgdInfo(input);
            if (xgdInfo.HasValue == false)
            {
                return false;
            }

            var rootSector = xgdInfo.Value.RootDirSector;
            var rootSize = xgdInfo.Value.RootDirSize;
            var rootOffset = (long)rootSector << 11;
            ...
            while (treeNodes.Count > 0)
            {
                ...
                var currentPosition = (xgdInfo.Value.BaseSector << 11) + currentTreeNode.DirectoryPos + currentTreeNode.Offset * 4;
                ...
                if ((attribute & 0x10) != 0)
                {
                    if (size > 0)
                    {
                        ...
                        info(new FileInfo
                        {
                            ...
                            Filename = filename,
                            Size = size,
                            StartSector = (int)(xgdInfo.Value.BaseSector + sector),
                            EndSector = (int)((xgdInfo.Value.BaseSector + sector) + ((size + 2047) >> 11) - 1),
                            InSlices = "N/A"
                        });
                    }
                }
                else
                {
                    if (size > 0)
                    {
                        var startSector = (int)(xgdInfo.Value.BaseSector + sector);
                        var endSector = (int)((xgdInfo.Value.BaseSector + sector) + ((size + 2047) >> 11) - 1);
